...
class ArtistDetailPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final artist = ref.watch(artistsProvider)
    ...
    if (artist == null) {
      return Scaffold(
        ...
        body: const EmptyState(
          ...
          message: 'The artist was not found on your device.',
    ...
    final artistSongs = songs
        .where((s) => s.artist.trim().toLowerCase() == artist.name.trim().toLowerCase())
        .toList();

    // Map MediaSong to Track
    final trackList = artistSongs.map((song) => Track(
      id: song.id,
      title: song.title,
      artist: song.artist,
        ...
        slivers: [
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  AppSpacing.heightXs,
                  Text(
                    '${artistSongs.length} song${artistSongs.length != 1 ? 's' : ''} · ${artist.albumCount} album${artist.albumCount != 1 ? 's' : ''}',
          ...
          if (trackList.isEmpty)
            const SliverFillRemaining(
              child: EmptyState(
                title: 'No songs',
                message: 'No scanned songs belong to this artist.',
